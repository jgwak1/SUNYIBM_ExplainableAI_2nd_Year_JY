...
class AppService(AppServiceInterface, BaseService):
    ...
    async def _write_reports(self):
        ...
        print(f"JY:  app.service.app_svc.::write_reports: {r_dir}\n", flush=True)
        ...
        for op in await self.get_service('data_svc').locate('operations'):
            ...
            if report:
                ...
                print("JY: app.service.app_svc.::write_reports -- Now saving out the report", flush=True)

                print(f"type(op): {type(op)}", flush=True)
                print(f"op: {op}", flush=True)

                # Change file name here?
                from datetime import datetime # Added by JY @ 2024-3-6
                # JY @ 2024-3-6: type(op) here is class OperationSchema in c_operation.py
                # JY @ 2024-3-6 could get 'adversary_id' from here, 
                #               according to "api::v2::managers::operation_api_manager::setup_operation" --  
                #                 adversary_id = data.pop('adversary', {}).get('adversary_id', '')
                # op_adversary = op.pop('adversary', {})
                # op_adversary_id = op_adversary.get('adversary_id', '')
                print(f"op_aversary: {op.adversary}", flush=True)
                print(f"op_adversary_id: {op.adversary.adversary_id}", flush=True) 
                # JY @ 2024-3-6: Also looked into "/home/etw0/Desktop/caldera/app/objects/c_adversary.py"
                
                operation_filename = f"operation__{op.adversary.adversary_id}__SAVED_{datetime.now().strftime('%Y-%m-%d_%H%M%S')}"
                await file_svc.save_file(operation_filename, report.encode(), r_dir)
