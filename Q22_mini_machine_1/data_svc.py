...
class DataService(DataServiceInterface, BaseService):
    ...
    async def locate(self, object_name, match=None):
        try:
            ...
            ###################################################################################################################            
            # folowing part added by zhan before
            if object_name == 'abilities':
                # print ('bp2 -- JY')
                all_operations = await self.get_service('data_svc').locate('operations')
                print ('here we write all the operations id into file for etw to control -- app/service/data_svc.py')
                l = []
                for i in all_operations:
                    l.append(i.id)
                    # print (i.id)
                #with open('/home/zshu1/tools/etw/tmp/operations.pkl','wb') as f:  # Modified by JY @ 2023-02-27
                with open('/home/etw0/Desktop/caldera/etw/tmp/operations.pkl','wb') as f:
                    pickle.dump(l,f)
    ...
    async def _load_adversaries(self, plugin):
        ...
        # import time # added by JY 

        for filename in glob.iglob('%s/adversaries/**/*.yml' % plugin.data_dir, recursive=True):
            
            # print("JY @ 2023-10-12 -- data_svc.py -- _load_adversaries", flush=True)
            # print(f"filename: {filename}", flush=True)
            # print(f'plugin.access: {plugin.access}\n', flush=True)
            
            # if "custom" in filename:
            #     print()


            # if "custom" in filename and\
            #     filename == "data/adversaries/custom_adversary_profile__APT_3__APT3_Phase3_Pattern_2__both__2023-10-22-17_56_41.yml":
            #     print()
            # elif "custom" in filename and\
            #     filename == "data/adversaries/custom_adversary_profile__None__None__stockpile__2023-10-22-18_00_30.yml":
            #     print()
    ...
    async def _load_abilities(self, plugin, tasks=None):
        ...
        for filename in glob.iglob('%s/abilities/**/*.yml' % plugin.data_dir, recursive=True):
            ...
            # print("JY @ 2023-10-12 -- data_svc.py -- _load_abilities", flush=True)
