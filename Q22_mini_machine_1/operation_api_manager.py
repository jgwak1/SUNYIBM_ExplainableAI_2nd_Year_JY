...
class OperationApiManager(BaseApiManager):
    ...
    async def setup_operation(self, data: dict, access: BaseWorld.Access): # JY @ 2024-3-6 : This is also notable func
        
        print("JY @ 2024-3-6 : Does the control flow comhere in -- api::v2::managers::operation_api_manager::setup_operation()", flush=True)
        ...
        data['adversary'] = await self._construct_and_dump_adversary(adversary_id) # JY : look at this
        fact_source_id = data.pop('source', {}).get('id', '')
        data['source'] = await self._construct_and_dump_source(fact_source_id)
        operation = OperationSchema().load(data) # JY : look at this
