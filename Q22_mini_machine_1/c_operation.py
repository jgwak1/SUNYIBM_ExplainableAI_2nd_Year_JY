...
class OperationSchema(ma.Schema):
    ...
    adversary = ma.fields.Nested(AdversarySchema()) # JY @ 2024-3-6 could get 'adversary_id' from here, acorrding to "v2::managers::peration_api_manager::setup_operation" --  adversary_id = data.pop('adversary', {}).get('adversary_id', '')
...
class Operation(FirstClassObjectInterface, BaseObject):
    ...
    # JY @ 2024-3-5 : set 'output=False' to 'output=True'
    # async def report(self, file_svc, data_svc, output=False):
    async def report(self, file_svc, data_svc, output=True):
        output = True # Just explicitly setted output=True
        try:
            ...
            for step in self.chain:
                ...
                if output and step.output:
                    print("JY @ 2024-3-5: objects::c_operation.py -- setting 'output = True' explicitly led here.", flush=True )
    ...
    async def event_logs(self, file_svc, data_svc, output=True):
    ...
    async def write_event_logs_to_disk(self, file_svc, data_svc, output=True):
    ...
    async def _convert_link_to_event_log(self, link, file_svc, data_svc, output=True):
