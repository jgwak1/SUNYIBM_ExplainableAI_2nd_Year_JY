            ...
            using (var TraceSession = new TraceEventSession(EventParseSessionName))
            {
                ...
                using (var EventSource = new ETWTraceEventSource(EventParseSessionName, TraceEventSourceType.Session))
                {
                    ...
                    EventParser.All += delegate (TraceEvent data)
                    {
                        ...
                        // ======================================================================================================
                        // JY @ 2024-1-24 -- 
                        // TODO: Write my custom filter which filters-out 

                        
                        if (data.ProcessName == "java"){
                                ProcessEventData = false;
                                Console.WriteLine("JY: filtering out event with ProcessName 'java'"); // logstash runs on java

                        }
                        else if (data.ProcessName == "SilkService"){
                                ProcessEventData = false;
                                Console.WriteLine("JY: filtering out event with ProcessName 'SilkService'");
                        }

                        // following two EventNames (File Provider) are explicitly excluded in "get_file_info()" in "FirstStep.py"
                        else if (data.EventName == "OperationEnd"){
                                ProcessEventData = false;
                                Console.WriteLine("JY: filtering out event with EventName 'OperationEnd'");
                        }


                        else if (data.EventName == "NameDelete"){
                                ProcessEventData = false;
                                Console.WriteLine("JY: filtering out event with EventName 'NameDelete'");
                        }
